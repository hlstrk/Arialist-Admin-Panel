...
function Main_Login() {
    ...
    if ($("#login__password").val() === "") {

        Toast.fire({
            icon: 'info',
            title: "Parola boş geçilemez"
        });
        return false;
    }
    ...
    $.ajax({
        ...
        Toast.fire({
            icon: 'error',
            title: 'Bir hata oluştu!", "Rahatta kalın, Halis kaptan düzeltecek...'});
...
function MesajGoster(islemSonucTuru, baslik, mesaj) {
    ...
    $("#DivResult").children("strong").text("");
    $("#DivResult").children("p").text("");
    ...
    $("#DivResult").addClass(_islemSonucTuru);
    $("#DivResult").children("strong").text(baslik);
    $("#DivResult").children("p").text(mesaj);
